...
/* [Mount Base Spec] */
mount_base_rotate = true;
mount_base_thickness = 1;
mount_base_length = 50;
...
difference()
{
    ...
    difference()
    {
        union()
        {
            ...
            minkowski()
            {
                if (mount_base_rotate)
                {
                    hull()
                    {
                        translate([0,0,(mount_base_length-smoothing*2)/2-(curve_height-smoothing*2)/2])
                        rotate([0,90,-90])
                            cube([mount_base_length-smoothing*2, mount_base_thickness, curve_thickness-smoothing*2], center=true);
